const filmHelper = {
  ...
  renderFilmTableItem: function (film, clazz) {
    return `
...
`;
  ...
  renderFilmTableItemAdult: function (film, clazz) {
    return `
    <p class=" block03__line-svg ${clazz}" > 
    <input type="checkbox" class="block_03_checkbox" id="${film.title.replaceAll(' ', '-')}">
        <label for="${film.title.replaceAll(' ', '-')}">
...
`;
  ...
  init: function () {
    ...
    for (let index = 0; index < this.films.length; index++) {
      ...
      if (!this.films[index].adult) {
        div.innerHTML += this.renderFilmTableItem(this.films[index], clazz);
      } else {
        div.innerHTML += this.renderFilmTableItemAdult(this.films[index], clazz);
      }
    }
  }
};
